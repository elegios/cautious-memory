...
func _update_tile_deco_above(tm: TileMapLayer, coord: Vector2i, types: Dictionary, cache: Array, apply_empty_probability: bool):
	# TODO(vipa, 2024-10-03): The logic here is rather messy and
	# annoying, but seems to work for now. Probably worth trying to
	# simplify the logic at some later point. Key points: assume that
	# we only match against 1) the thing we're an edge from, 2)
	# DECORATION, which we interpret as being anything we're not an
	# edge from, and 3) the water category, *nothing else*. Edges are
	# only allowed if the current tile isn't of that type and all
	# peering bits match (modulo different interpretation of
	# DECORATION). Finally, if water is in any peering bit, then the
	# current tile must be water, otherwise the tile doesn't match.
	...
	for t in cache[-1]:
		...
		var edge_peer := []
		for peering in t[3]:
			if not t[3][peering].has(-1) and not t[3][peering].has(0):
				edge_peer = t[3][peering]
				break
		var edge_type := edge_peer.max()
		# NOTE(vipa, 2024-10-03): Follow the index order in the
		# tileset
		if edge_type != null and edge_type > type:
			continue
		for peering in t[3]:
			if t[3][peering].has(0):
				# NOTE(vipa, 2024-10-03): Matching against water,
				# should be interpreted as checking if the current
				# tile is water
				if t[3][peering].has(type):
					score += 3
				else:
					valid = false
					break
			elif t[3][peering].has(-1):
				# NOTE(vipa, 2024-10-03): It's "decoration" which we
				# interpret as "not whatever we're an edge of"
				if not edge_peer.has(types[tm.get_neighbor_cell(coord, peering)]):
					score += 3
				else:
					valid = false
					break
			elif t[3][peering].has(type):
				# NOTE(vipa, 2024-10-03): Not water, but a self-match,
				# which isn't allowed
				valid = false
				break
			elif t[3][peering].has(types[tm.get_neighbor_cell(coord, peering)]):
				score += 3
			else:
				valid = false
				break

		# NOTE(vipa, 2024-10-03): We only draw decoration on the extra
		# layer when a majority of the peering bits do not match
		# ourselves
		if not valid:
			continue
		# score -= self_match*2
...
func _update_tile_immediate(tm: TileMapLayer, coord: Vector2i, ts_meta: Dictionary, types: Dictionary, cache: Array) -> void:
	...
	var deco_tm := tm.get_child(0) as TileMapLayer if tm.get_child_count() else null
	...
	if deco_tm:
		placement = _update_tile_deco_above(tm, coord, types, cache, true)
		if placement:
			deco_tm.set_cell(coord, placement[0], placement[1], placement[2])
